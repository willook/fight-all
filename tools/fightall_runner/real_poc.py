...
from .export_league import INITIAL_RATING, K_FACTOR, _apply_elo
...
def _poc_models() -> list[dict[str, Any]]:
    return [
        {
            "id": OPENAI_MODEL_ID,
            "name": "GPT-4.1 Nano",
...
def _player_prompt(item: dict[str, str], self_model: dict[str, Any], opponent: dict[str, Any], position: str) -> str:
    if item["languageCode"] == "ko":
        ...
        scenario = (
            "늑대인간 게임의 마지막 토론입니다. 당신은 마을 팀입니다. "
            "상대 발언의 허점을 짚고, 왜 당신의 투표 논리가 더 설득력 있는지 말하세요."
        )
    else:
        language_instruction = "Answer in English."
        scenario = (
            "This is the final debate in a Werewolf game. You are on the village team. "
...
def _judge_prompt(
    ...
    if item["languageCode"] == "ko":
        language_instruction = "한국어 늑대인간 토론을 판정하세요."
    else:
        language_instruction = "Judge this English Werewolf debate."
    return (
        f"{language_instruction}\n"
...
def _match_summary(judgement: dict[str, Any], model_index: dict[str, dict[str, Any]]) -> str:
    ...
    if winner_id is None:
        return f"The judge marked this Werewolf PoC match as a draw. {reason}"
    return f"{model_index[winner_id]['name']} won this Werewolf PoC match. {reason}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a real OpenAI/Gemini GLADI PoC league.")
